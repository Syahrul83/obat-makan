...
class ObatController extends Controller
{
    public function index()
    {
        ...
        $link  = 'obat';
        $page  = 'data-obat';
        return view('Kasir.data-obat.main',compact('title','page','link'));
    }
    ...
    public function tambah()
    {
        ...
        return view('Kasir.data-obat.form-obat',compact('title','page','link','jenis_obat','supplier_obat','pabrik_obat','golongan_obat','margin_obat','nilai_ppn'));
    }
    ...
    public function edit($id) 
    {
        ...
        $margin_obat    = MarginObat::firstOrFail();
        $nilai_ppn      = PersenPpn::firstOrFail();
        return view('Kasir.data-obat.form-obat',compact('title','link','page','jenis_obat','supplier_obat','row','obat_detail','id','komposisi_obat','pabrik_obat','golongan_obat','margin_obat','nilai_ppn'));
    }

    public function delete($id) 
    {
        Obat::where('id_obat',$id)->update(['status_delete' => 1]);
        return redirect('/kasir/data-obat')->with('message','Berhasil Hapus Obat');
    }
    ...
    public function save(Request $request) 
    {
        ...
        return redirect('/kasir/data-obat')->with('message',$message);
    }
    ...
    public function obatDetail($id)
    {
        ...
        $link  = 'obat';
        $page  = 'data-obat';
        return view('Kasir.data-obat.obat-detail',compact('title','page','link','id'));    
    }

    public function deleteObatDetail($id,$id_detail)
    {
        ObatDetail::where('id_obat_detail',$id_detail)->delete();
        return redirect('/kasir/data-obat/lihat-supplier/'.$id)->with('message','Berhasil Hapus Data'); 
    }
    ...
    public function komposisiObat($id)
    {
        ...
        $link  = 'obat';
        $page  = 'data-obat';
        return view('Kasir.data-obat.komposisi-obat',compact('title','link','page','id'));    
    }

    public function deleteKomposisiObat($id,$id_detail)
    {
        KomposisiObat::where('id_komposisi_obat',$id_detail)->delete();
        return redirect('/kasir/data-obat/komposisi-obat/'.$id)->with('message','Berhasil Hapus Data'); 
    }
    ...
    public function rekapObat(Request $request)
    {
        ...
        $spreadsheet->getActiveSheet()->setCellValue('A4','Tanggal : '.human_date($from).' - '.human_date($to));
        ...
        $spreadsheet->getActiveSheet()->setCellValue('E7','Bentuk Sediaan Obat');
        ...
        $get   = Obat::join('jenis_obat','obat.id_jenis_obat','=','jenis_obat.id_jenis_obat')
        ...
        foreach ($get as $key => $value) {
            ...
            $spreadsheet->getActiveSheet()->setCellValue('B'.$cell,$value->nama_pabrik);
            $spreadsheet->getActiveSheet()->setCellValue('C'.$cell,$value->nama_obat);
            $spreadsheet->getActiveSheet()->setCellValue('D'.$cell,$value->nama_golongan);
            $spreadsheet->getActiveSheet()->setCellValue('E'.$cell,$value->nama_jenis_obat);
            $spreadsheet->getActiveSheet()->setCellValue('F'.$cell,human_date($value->tanggal_expired));
            $spreadsheet->getActiveSheet()->setCellValue('G'.$cell,$value->stok_obat.' '.$value->satuan_obat);
            $spreadsheet->getActiveSheet()->setCellValue('H'.$cell,$value->dosis_satuan);
            $spreadsheet->getActiveSheet()->setCellValue('I'.$cell,$value->harga_modal);
            $spreadsheet->getActiveSheet()->setCellValue('J'.$cell,$value->harga_modal_ppn);
            $spreadsheet->getActiveSheet()->setCellValue('K'.$cell,$value->hja_upds);
            $spreadsheet->getActiveSheet()->setCellValue('L'.$cell,$value->hja_resep);
            $spreadsheet->getActiveSheet()->setCellValue('M'.$cell,$value->hja_relasi);
            $count++;
            $cell++;
        }
