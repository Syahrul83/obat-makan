...
class PersenPpnController extends Controller
{
    ...
    {
        $title = 'Persen PPN | Kasir';
        $link  = 'data-master';
        $page  = 'data-ppn';

        return view('Kasir.data-ppn.main',compact('title','link','page'));
    }

    public function edit($id)
    {
        $title = 'Form Persen PPN | Kasir';
        $link  = 'data-master';
        $page  = 'data-ppn';
        $row   = PersenPpn::where('id_persen_ppn',$id)->firstOrFail();

        return view('Kasir.data-ppn.form-data-ppn',compact('title','link','page','row'));
    }
    ...
    public function save(Request $request)
    {
        ...
        return redirect('/kasir/data-ppn')->with('message','Berhasil Update PPn');
    }
}
